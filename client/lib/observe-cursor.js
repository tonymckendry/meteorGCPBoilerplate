    ...
    if (handle.ready()) {
        ...
        Tracker.nonreactive(() => {
            action(`${actionPrefix}: initial fetch`, allDocs => {
                ...
                loading = false // eslint-disable-line no-param-reassign
            })(cursor.fetch())
        })
        ...
        cursor.observe({
            ...
            addedAt: action(`${actionPrefix}: document added`, (document, atIndex) => {
                console.log('something was added: ' + actionPrefix) //eslint-disable-line no-console
                observableArray.splice(atIndex, 0, createState(observerState, document))
            }),
            changedAt: action(
                `${actionPrefix}: document changed`,
                (newDocument, oldDocument, atIndex) => {
                    console.log('something was changed: ' + actionPrefix) //eslint-disable-line no-console
                    if (!isUndefined(observerState)) {
            ...
            removedAt: action(`${actionPrefix}: document removed`, (oldDocument, atIndex) => {
                console.log('something was removed: ' + actionPrefix) //eslint-disable-line no-console
